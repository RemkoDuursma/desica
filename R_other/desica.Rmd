...
- During severe drought, water loss continues due to the minimum conductance ((see here for a review in progress on that)[https://github.com/remkoDuursma/g0paper])
...
\
Now it would be possible to solve equations (@dpsildt) and (@dpsisdt) numerically (with a Runge-Kutta integrator or similar), but this is not desirable because a) the computational overhead, b) the headache with implementing a full numerical solution, not to mention the potential instabilities given the various dependencies on water potential.
...
Where $\Psi_L(t)$ is the solution for the current timestep (using the value from the previous timestep, *t* - 1), $\Delta t$ is the timestep. 
